    ...
    override func spec() {
        describe("detail view controller spec") {
            ...
            beforeSuite {
                ...
                models = (apiClient.fetchJSON(from: topGamesJsonFileName) as? [Any])?.toModels() ?? []
            ...
            it("should have a label to show the game viewers") {
                ...
                expect(controller?.viewersLabel.text).to(equal(LocalizedUtil.Text.detailsViewersLabelText + String(controller?.detailViewModel?.getViewers() ?? 0)))
